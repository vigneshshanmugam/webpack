	...
	compiler.plugin("after-emit", function(compilation, callback) {
		...
		compilation.chunks.forEach(function(chunk) {
			chunk.files.forEach(function(file) {
				...
				assetsByFile[file] = obj;

				chunk.ids.forEach(function(id) {
					assetsByFile[file].chunks.push(id);
				});
				if(chunk.name) {
					assetsByFile[file].chunkNames.push(chunk.name);
					if(assetsByChunkName[chunk.name]) {
						assetsByChunkName[chunk.name] = [].concat(assetsByChunkName[chunk.name]).concat([file]);
					} else {
						assetsByChunkName[chunk.name] = file;
					}
				}
			});
		});
		...
		var entrypoints = Object.keys(compilation.entrypoints).map(function(ep) {
			...
			files.forEach(function(file) {
				...
				entrypoint.isOverSizeLimit = !!entrypoint.assets[file].isOverSizeLimit;
			});
